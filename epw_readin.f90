...
#include "f_defs.h"
  ...
  namelist / inputepw / &
       ...
       degaussw, fsthick, eptemp, efsweep,  nsmear, delta_smear, eminabs, emaxabs,      &
       ...
       degaussq, delta_qsmear, nqsmear, nqstep, constepm , perphon, groupq,    &
       ldoubledelta, egap0, ltetra_elec, ltetra_phon
  !
  ! amass    : atomic masses
  ...
  !
  ! added by BL
  ! egap0 : the imposed band gap (eV) when calculating occupation numbers
  ! ldoubledelta : if .true., double delta approximation is used for phonon
  ! ltetra_elec: if .true. use tetrahedron method for electron selfenergy
  ! ltetra_phon: if .true. use tetrahedron method for phonon selfenergy
...
100 CALL errore ('epw_readin', 'reading title ', abs (ios) )
  ...
  !
  constepm     = .false. !! constant ep-matrix element for debug
  perphon      = .false. !! output selfen_elec per phonon
  groupq       = .false. !! group phonons according to extra column of flags in meshfq.dat
  amass(:)     = 0.d0
  iverbosity   = 0
  elph         = .false.
  tshuffle     = .false.
...
200 CALL errore ('epw_readin', 'reading input_epw namelist', abs (ios) )
  ...
  IF (degaussw(1)*4.d0.gt.fsthick) CALL errore ('epw_readin', &
...
!       'rand_q meant to be run with fly=true', 1)
  ...
  DO i = 1, ndegxx
     IF (degaussw(i) .gt. 0.d0) THEN
        ndeg = i
        ! from eV to Ryd
        degaussw(i) = degaussw(i) / ryd2ev
     ENDIF
  ENDDO
  
  IF (efsweep(1) .le. -1.0d5) then
        isefsweep = 0 !! no efsweep input, will go with internally computed Fermi level
        nefsweep =1
  ELSE 
        isefsweep = 1 !! sweep through a bunch of chemical potentials
          DO i = 1, nefsxx
             IF (efsweep(i) .gt. 0.d0) THEN
                nefsweep = i
                !! from eV to Ryd
                efsweep(i) = efsweep(i) / ryd2eV
             ENDIF
          ENDDO
  ENDIF
  
  write(stdout, *)
  write(stdout, *) 'The number of degauss parameters (following line) and they are (eV): '
  write(stdout, *) trim(prefix)
  write(stdout, *) ndeg
  do i = 1 , ndeg
        write(stdout, 324) degaussw(i)*ryd2ev
  enddo

  write(stdout, *)
  write(stdout, *)
  write(stdout, *) 'The temperatures studied: '
        write(stdout, *) neptemp
  do i = 1 , neptemp
        write(stdout, 324) eptemp(i) / 0.000086173423 * ryd2eV
  enddo
  
  write(stdout, *)
  write(stdout, *)
  if (isefsweep .eq. 1) then
          write(stdout, *) 'The chemical potentials studied: (eV)'
                write(stdout, *) nefsweep
          do i = 1 , nefsweep
                write(stdout, 324) efsweep(i) * ryd2ev
        enddo
        efprime = ( maxval ( efsweep(:) ) + minval ( efsweep(:) ) )/2.0d0 !! new reference ef for integration window
        fsthickprime =  ( maxval ( efsweep(:) ) - minval ( efsweep(:) ) )/2.0d0 + fsthick !! new effective fsthick
  else
        write(stdout, *) 'The chemical potential is not specified, will use internal value: '
  endif
  if (egap0 .ne. 0) then
      egap0=egap0 / ryd2eV !switch to Ryd
      write(stdout, *) 'The bang gap of', egap0*ryd2eV,'is imposed'  
  endif
