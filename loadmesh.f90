...
#include "f_defs.h"
  ...
  USE epwcom,    ONLY : filkf, nkf1, nkf2, nkf3, skf1, skf2, skf3, &
       rand_k, rand_nk, mp_mesh_k, ltetra_phon, lower_bnd, upper_bnd
  ...
  real(kind=DP), ALLOCATABLE :: xkf_(:,:), wkf_(:), xkf_tmp(:,:), &
       wkf_tmp(:)
  integer :: ik, ikk, ikq, rest, &
       ipol, i, j, k, nrot, tipo,crystorcart, ierr
 ...
 IF (mpime .eq. ionode_id) THEN
    ...
    IF (exst) THEN ! load from file (crystal coordinates)
       ...
       READ(iunkkf, *) nkstotf, crystorcart !! addition: mesh in cart 0 or crystal 1
       !
       if (crystorcart .eq. 0) then
           WRITE(stdout,*) "Input is in cartesian coord, transforming into crystal ..."
        endif
    ...
    ELSEIF ( (nkf1.ne.0) .and. (nkf2.ne.0) .and. (nkf3.ne.0) ) THEN ! generate grid
       IF (mp_mesh_k) THEN !generate mesh in irreducible wedge
          ...
          nkstotf = 2 * nkstotf
          !
       ELSE !generate uniform k mesh in full zone
          !
...
#ifdef __PARA
 ENDIF
 WRITE(stdout,'(/5x,a)') 'Here reached,1'
 ...
 WRITE(stdout,'(/5x,a)') 'Here reached,3'
 ...
 WRITE(stdout,'(/5x,a)') 'Here reached,4'
 !
#else
 !
...
#include "f_defs.h"
  ...
  USE epwcom,    ONLY : filqf, nqf1, nqf2, nqf3, sqf1, sqf2, sqf3, &
       rand_q, rand_nq, mp_mesh_q, groupq, nqgroup
  USE el_phon,   ONLY : xqf, wqf, nksqf, nxqf, gpid
  USE pwcom,     ONLY : at, bg, ibrav, s, sname, symm_type
  USE symme,     ONLY : t_rev, time_reversal
  implicit none
  !
  ...
  integer, PARAMETER :: iunqf  = 77
  ! unit with fine k point mesh (crys coord)
  integer :: iq , i, j, k, nrot, tipo,crystorcart
