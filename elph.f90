  ...
  REAL(KIND=DP), ALLOCATABLE ::&
       ...
       gpid(:),                 &!  q-shell id for grouped phonon output for selfen_elec
       ...
       ekk_all(:,:),           &
       sigmai_all_q(:,:,:,:,:),    &
       sigmai_all_gpq_abs(:,:,:,:,:,:,:),    & ! this is to store the self energy contributed by per phonon group per phonon branch
       sigmai_all_gpq_emi(:,:,:,:,:,:,:),    &
       xkf_all(:,:),          &
       jdos(:),                &
