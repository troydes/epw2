  ...
  INTEGER :: ngaussw, nw, selfen_type, nbndsub, nbndskip, num_iter, iprint, &
       ...
       sqf1, sqf2, sqf3, skf1, skf2, skf3, nqsmear, nqstep, neptemp, ndeg, nefsweep, isefsweep, &
       nqgroup
  ...
  INTEGER :: nwanxx = 200, ntempxx = 100, ndegxx = 100, nefsxx = 100
  ! parameter used in writing prefix.win file.  Maximum number of wannier functions
  REAL (KIND=DP) :: fsthick, fsthickprime, efprime, wmin, wmax, ecutse, dis_win_min, dis_win_max, &
       dis_froz_min, dis_froz_max, delta_smear, eminabs, emaxabs, deltaeabs, eps_acustic, &
       degaussq, delta_qsmear, egap, egap0
  REAL (KIND=DP), DIMENSION(100) :: degaussw
  ...
  logical :: phinterp , elinterp, tphases, epstrict, tshuffle2, elecselfen, phonselfen, &
       ...
       band_plot, constepm, perphon, groupq, ldoubledelta
  ...
  ! ldoubledelta: if .true., use double delta approximation in phonon self
  ! energy
  !
  ! variables for tetrahedron integration
  logical :: ltetra_elec, ltetra_phon  !if .true., use tetrahedra method for
  !electron/phonon selfenergy calculation
  ...
  REAL (kind=DP), dimension(100) :: eptemp 
  REAL (kind=DP), dimension(100) :: efsweep 
  REAL (kind=DP), allocatable :: etf_global(:,:) !(nbndsub,nkstotf) a global copy of electronic eigenvalues, for tetrahedron
  integer :: iswitch
...
!variables for tetrahedron integration - Added by BL
INTEGER :: lower_bnd, upper_bnd
!the lower and upper bound of k-point index of this pool
