...
100  call errore ('elphon', 'opening file'//filelph, abs (ios) )
     rewind (iuelph)
     WRITE (iuelph, '(3f15.8,f8.3,2i8)') xq, degausswi, ngaussw, 3 * nat
  ...
  DO isig = 1,3
    ...
    degaussw_ = degausswi * 10.d0**float(isig-2) 
    !
...
!  CALL efermig (etf, nbndsub, nksf, nelec, wkf, degausswi, ngaussw, ef1)
  ef1 = efermig(etf, nbndsub, nksf, nelec, wkf, degausswi, ngaussw,0,isk)
  DOsef = dos_ef (ngaussw, degausswi, ef1, etf, wkf, nksf, nbndsub)
    ...
    IF (tfermikk.and.tfermikq) then
      ...
      DO ibnd = 1, nbndsub
        ...
        DO jbnd = 1, nbndsub
          ...
          ekq = etf (jbnd, ikq) - ef1
          !
          w0g1 = w0gauss( -ekk/degausswi, ngaussw)/degausswi
          w0g2 = w0gauss( -ekq/degausswi, ngaussw)/degausswi
          !
  ...
  ENDDO
  !
  WRITE (6, 9000) degausswi, ngaussw
  WRITE (6, 9005) dosef, ef1 * ryd2ev
  WRITE (6, 9006) phase_space
  IF (iuelph.ne.0) then
     WRITE (iuelph, *) 
     Write (iuelph, 9000) degausswi, ngaussw
