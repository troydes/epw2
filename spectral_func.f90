...
#include "f_defs.h"
  ...
  real(kind=DP) :: g2, ekk, ekq, wq, ef0, wgq, wgkq, ww, dw, &
     ...
     sigmai(nbndsub, nksf, nw), a(nw, nksf), degausswi
  !
  ! variables for collecting data from all pools in parallel case 
  !
  integer :: nksqtotf
  real(kind=DP), allocatable :: etf_all(:,:), a_all(:,:)
  !
  ...
  ENDIF
  !
!  CALL efermig(etf, nbndsub, nksf, nelec, wkf, degausswi, ngaussw, ef0)
  ef0 = efermig(etf, nbndsub, nksf, nelec, wkf, degausswi, ngaussw,0,isk)
  DOsef = dos_ef (ngaussw, degausswi, ef0, etf, wkf, nksf, nbndsub)
  !   N(Ef) in the equation for lambda is the DOS per spin
  DOsef = dosef / two
  !
  WRITE (6, 100) degausswi, ngaussw
          ...
          DO ibnd = 1, ibndmax-ibndmin+1
            ...
            DO jbnd = 1, ibndmax-ibndmin+1
              ...
              DO iw = 1, nw
                ...
                !
                weight =  two * wqf(iq) *  &
                  ( (       wgkq + wgq ) / ( ww - ( ekq - wq ) - ci * degausswi )  +  &
                    ( one - wgkq + wgq ) / ( ww - ( ekq + wq ) + ci * degausswi ) ) 
                !
