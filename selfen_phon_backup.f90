...
#include "f_defs.h"
  ...
  USE epwcom,    ONLY : nbndsub, lrepmatf, iunepmatf, fsthick, &
                        ...
                        nsmear, delta_smear, eig_read, eps_acustic, egap, egap0
...
#include "f_defs.h"
  ...
  use epwcom,    ONLY : nbndsub, lrepmatf, iunepmatf, fsthick, &
                        ...
                        neptemp, ldoubledelta, egap, egap0
  ...
  DO ismear = 1, ndeg
     ...
     !   no spin-polarized calculation here
   do iefs = 1, nefsweep
         ef0 = efsweep(iefs) !! override for sweeping ef
     ! ef0 = efermig(etf,nbndsub,nksf,nelec,wkf,degaussw0,ngaussw,0,isk)
     dosef = dos_ef (ngaussw, degaussw0, ef0, etf, wkf, nksf, nbndsub)
     !
     !   N(Ef) in the equation for lambda is the DOS per spin
...
!! commented out so I can sweep efs
     CALL start_clock('PH SELF-ENERGY')
     !
     fermicount = 0
     gamma = zero
     ...
     DO ik = 1, nksqf
        ...
        IF ( ( minval ( abs(etf (:, ikk) - ef0) ) .lt. fsthick ) .and. &
           ...
           DO imode = 1, nmodes
              ...
              DO ibnd = 1, ibndmax-ibndmin+1
                 !For testing purpose
                 !IF (etf (ibndmin-1+ibnd, ikk) .ge. ef) cycle
                 !  the fermi occupation for k
                 ekk = etf (ibndmin-1+ibnd, ikk) - ef0
                 ! impose the band gap
                 ...
                 DO jbnd = 1, ibndmax-ibndmin+1
                    !For testing purpose
                    !IF (etf(ibndmin-1+jbnd,ikq) .ge. ef) cycle
                    !  the fermi occupation for k+q
                    ekq = etf (ibndmin-1+jbnd, ikq) - ef0
                    ! impose the band gap
...
100 format(5x,'Gaussian Broadening: ',f7.3,' Ry, ngauss=',i4)
101 format(5x,'DOS =',f10.6,' states/spin/Ry/Unit Cell at Ef=',f10.6,' eV')
102 format(5x,'lambda( ',i3,' )=',f13.7,'   gamma=',f13.7,' meV','   omega=',f13.7,' meV')
