 ...
 USE mp_global, ONLY : nproc, my_pool_id, nproc_pool,    &
  ...
  !--------------------------------------------------------------------
!  subroutine poolgather3 (nsize, nkstot, nks, ndeg, neptemp, nefsweep, f_in, f_out)
!  !--------------------------------------------------------------------
!  !
!  !  gather the kpoints and the electronic eigenvalues
!  !  across the pools 
...
!  implicit none
!  !
!  INTEGER :: nsize, nkstot, nks, ndeg, neptemp, nefsweep
!    ! first dimension of vectors f_in and f_out
!    ! number of k-points per pool
!    ! total number of k-points
!  REAL (KIND=DP) :: f_in(nsize,nks, ndeg, neptemp, nefsweep), f_out(nsize,nkstot, ndeg, neptemp, nefsweep)
!    ! input ( only for k-points of mypool )
!    ! output  ( contains values for all k-point )
!  INTEGER :: rest, nbase, nkst
!    ! the rest of the integer division nkstot / npo
...
!#endif
!  !
!  end subroutine poolgather3
!  !
!
!
!  !--------------------------------------------------------------------
!  subroutine poolgather4 (nsize, nkstot, nks, ndeg, neptemp, nefsweep, nmodes, f_in, f_out)
!  !--------------------------------------------------------------------
!  !
!  !  gather the kpoints and the electronic eigenvalues
!  !  across the pools 
...
!  implicit none
!  !
!  INTEGER :: nsize, nkstot, nks, ndeg, neptemp, nefsweep, nmodes
!    ! first dimension of vectors f_in and f_out
!    ! number of k-points per pool
!    ! total number of k-points
!  REAL (KIND=DP) :: f_in(nsize,nks, ndeg, neptemp, nefsweep, nmodes), f_out(nsize,nkstot, ndeg, neptemp, nefsweep, nmodes)
!    ! input ( only for k-points of mypool )
!    ! output  ( contains values for all k-point )
!  INTEGER :: rest, nbase, nkst
!    ! the rest of the integer division nkstot / npo
